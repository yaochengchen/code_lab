...
int filter_fre(int thread, double x[4096], double f_x[4096]);
...
double ***x = new_Array3D<double>(N_Threads, 8, 4096);
 Double_t ***new_x = new_Array3D<Double_t>(N_Threads, 8, 4096);
...
//cout<<"input:   "<<nt_j<<endl;
     ...
     if(rough_theta<6.6){
     	...
     	upper_phipos = min(181, m_phipos + 5);//[,)
     	lower_phipos = max(0, m_phipos - 4);
     ...
     //high elevation events
     	...
     	upper_phipos = min(18, m_phipos + 2);
     	lower_phipos = max(0, m_phipos - 1);
     	
     	upper_thetapos = min(36, m_thetapos + 6);
     	lower_thetapos = max(0, m_thetapos - 5);    
...
double a1 = 1. - a0;
 	for(int i=0;i<1500;i++)
		{
		new_x[i] = ((i<Max_i-200)||(i>Max_i+200))?0.:f_x[i];//(a0 - a1*cos(2.*PI*(i-Max_i+200)/(400.-1.)))*f_x[i];
		}
...
void get_noise_level(double noise_mag[8][2049]){
   ...
   FILE *fpin = fopen("./noise_level.txt","r");// 600m
   cout<<fpin<<endl;
