...
//int main(int argc, char *argv[]) {
 void fitting(){
 	initial();
 	double measured_delays[4] = {-248.976, -248.312, -247.264, -247.248}; // delays between main and second pulses
 	...
 	TFile *f_ntuple_main_V = new TFile("main_V_20221228.root","READ");
	...
	TFile *f_ntuple_main_H = new TFile("main_H_20221228.root","READ");
	...
	TFile *f_ntuple_second_H = new TFile("second_H_20221228.root","READ");
	...
	for(int entry=0; entry<ntuple_main_V->GetEntries(); entry++){
		...
		int theta_pos = round((_ntuple_theta-2.4)/0.1);
		int phi_pos = round((_ntuple_phi-(-25.1))/0.6);
	...
	for(int entry=0; entry<ntuple_second_H->GetEntries(); entry++){
		...
		int theta_pos = round((_ntuple_theta-(-5.5))/0.1);
		int phi_pos = round((_ntuple_phi-(-25.1))/0.6);
		int pair_pos = round(_ntuple_pair);
	...
	TFile *f_out = new TFile("/media/cyc/For_Linux/fitting_result_rough_delay_direct_HV_reflected_20221228.root", "recreate");
	...
	//tr_cr->Branch("reflected_delta_t", _reflected_delta_t, "reflected_delta_t[6]/D");
 	...
 	for(double theta=2.4; theta<=5.4; theta+=0.1){// 2.2 3.1 0.02  2.4 5.4 0.1
 		cout<<"theta   "<<theta<<endl;
 		for(double phi=-25.1; phi<=-24.6; phi+=0.02){// -25.1 -24.7 0.01  -25.1 -24.6 0.02
 			
 			int main_theta_pos = round((theta-2.4)/0.1);
			int main_phi_pos = round((phi-(-25.1))/0.6);
			...
			if(main_phi_pos>1) main_phi_pos = 1;
 			...
 			//cout<<"phi   "<<phi<<endl;
 			for(double R=10000; R<=500000; R+=5000){//50000 400000 100   10000 500000 5000
 				//cout<<"R   "<<R<<endl;
 				...
 				for(double z=678.2; z<=698.2; z+=0.05){// 678.2 698.2 0.05
 					...
 					for(int ant=0; ant<4; ant++){
 						...
 						chi_square += (pow((delay[ant]-measured_delays[ant])/0.138,2));
 						//cout<<delay[ant]<<endl;
 					}
 					...
 					for(int pair = 0; pair <6; pair++){
    					...
    					int j_V = j + 4;
							...
							// direct signal
          					...
          					chi_square += (pow((delta_t_V[pair]-delta_t_theta_phi_main_V[main_theta_pos][main_phi_pos][pair])/0.053,2));
          					chi_square += (pow((delta_t_H[pair]-delta_t_theta_phi_main_H[main_theta_pos][main_phi_pos][pair])/0.070,2));
          					...
          					int second_theta_pos = round((reflected_angle[1]-(-5.5))/0.1);//antenna 2
          					if(second_theta_pos<0) second_theta_pos = 0;
							if(second_theta_pos>30) second_theta_pos = 30;
          					chi_square += (pow((delta_t_second_H[pair]-delta_t_theta_phi_second_H[second_theta_pos][main_phi_pos][pair])/0.143,2));
