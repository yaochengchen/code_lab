...
//int main(int argc, char *argv[]) {
 void fitting(){
 	initial();
 	double measured_delays[4] = {-63.176, -62.960, -62.504, -62.464}; // delays between main and second pulses
 	...
 	TFile *f_ntuple_main_V = new TFile("main_V_20230327.root","READ");
	...
	TFile *f_ntuple_main_H = new TFile("main_H_20230327.root","READ");
	...
	TFile *f_ntuple_second_H = new TFile("second_H_20230327.root","READ");
	...
	TFile *f_ntuple_second_V = new TFile("second_V_20230327.root","READ");
	...
	for(int entry=0; entry<ntuple_main_V->GetEntries(); entry++){
		...
		int theta_pos = round((_ntuple_theta-(-0.8))/0.1);
		int phi_pos = round((_ntuple_phi-(-57))/0.6);
	...
	for(int entry=0; entry<ntuple_second_H->GetEntries(); entry++){
		...
		int theta_pos = round((_ntuple_theta-(-3.9))/0.1);
		int phi_pos = round((_ntuple_phi-(-57))/0.6);
		int pair_pos = round(_ntuple_pair);
	...
	TFile *f_out = new TFile("/media/cyc/For_Linux/fitting_result_rough_delay_direct_HV_reflected_20230327.root", "recreate");
	...
	//tr_cr->Branch("reflected_delta_t", _reflected_delta_t, "reflected_delta_t[6]/D");
 	...
 	for(double theta=-0.8; theta<=2.2; theta+=0.1){// -0.2 0.6 0.02  -0.8 2.2 0.1
 		cout<<"theta   "<<theta<<endl;
 		for(double phi=-57; phi<=-56; phi+=0.02){// -56.9 -56.5 0.01  -57 -56 0.02
 			
 			int main_theta_pos = round((theta-(-0.8))/0.1);
			int main_phi_pos = round((phi-(-57))/0.6);
 			...
 			for(double R=10000; R<=500000; R+=5000){//50000 400000 100   10000 500000 100
 				...
 				for(double z=678.2; z<=698.2; z+=0.05){// 678.2 698.2 0.05
 					...
 					for(int ant=0; ant<4; ant++){
 						...
 						chi_square += (pow((delay[ant]-measured_delays[ant])/0.106,2));
 						//cout<<delay[ant]<<endl;
 					}
 					...
 					for(int pair = 0; pair <6; pair++){
    					...
    					int j_V = j + 4;
							...
							// direct signal
          					...
          					chi_square += (pow((delta_t_V[pair]-delta_t_theta_phi_main_V[main_theta_pos][main_phi_pos][pair])/0.53,2));
          					chi_square += (pow((delta_t_H[pair]-delta_t_theta_phi_main_H[main_theta_pos][main_phi_pos][pair])/0.53,2));
          					...
          					int second_theta_pos = round((reflected_angle[1]-(-3.9))/0.1);//antenna 2
          					if(second_theta_pos<0) second_theta_pos = 0;
							if(second_theta_pos>30) second_theta_pos = 30;
          					chi_square += (pow((delta_t_second_H[pair]-delta_t_theta_phi_second_H[second_theta_pos][main_phi_pos][pair])/0.111,2));
          					chi_square += (pow((delta_t_second_V[pair]-delta_t_theta_phi_second_V[second_theta_pos][main_phi_pos][pair])/0.111,2));
