...
//int main(int argc, char *argv[]) {
 void fitting(){
 	initial();
 	double measured_delays[4] = {-225.952, -225.184, -224.640, -224.368};// this is wrong, maybe due to finding X-cor. maximum instead of minimum
 	...
 	TFile *f_ntuple_main_V = new TFile("main_V_.root","READ");
	...
	TFile *f_ntuple_main_H = new TFile("main_H_.root","READ");
	...
	TFile *f_ntuple_second_H = new TFile("second_H_.root","READ");
	...
	TFile *f_out = new TFile("/media/cyc/For_Linux/_1fitting_result_fine_delay_direct_HV_reflected.root", "recreate");
	...
	//tr_cr->Branch("reflected_delta_t", _reflected_delta_t, "reflected_delta_t[6]/D");
 	...
 	for(double theta=2.15; theta<=2.75; theta+=0.02){//2.45 2.75 0.02  1.5 3.5 0.1
 		cout<<"theta   "<<theta<<endl;
 		for(double phi=-22.75; phi<=-22.35; phi+=0.01){//-22.75 -22.35 0.01   -22.8 -22.4 0.02
			...
			if(main_phi_pos>1) main_phi_pos = 1;
 			...
 			//cout<<"phi   "<<phi<<endl;
 			for(double R=50000; R<=400000; R+=100){//50000 400000 100   10000 500000 100
 				//cout<<"R   "<<R<<endl;
