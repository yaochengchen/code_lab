...
for(int high_low=0; high_low<2; high_low++){          
     ...
     if(high_low==0){
     	...
     	upper_phipos = 105;// -16.5
     	lower_phipos = 103;// -17.1
     	
     	upper_thetapos = 126;//  -3.5 36; 5.5
     	lower_thetapos = 95;// -6.5 5; 2.5 95
     ...
     //high elevation events
     	...
     	is_high = 1;
     	int m_phipos = (int) round((rough_phi - (-25.) - 3.)/6.) + 9;// -3 because of round
...
void low_elevation_reconstruction(int n_thread, int nt_j, double R, int lower_thetapos, int upper_thetapos, int lower_phipos, int upper_phipos, double max_theta[3], double max_phi[3], double max[3]){
	...
	TFile *f_ntuple = new TFile("_main_V_20200731.root","RECREATE");
...
get_cross_correlation(n_thread, nt_j);
	...
	for(int k=0; k<6; k++){
		//delta_t_theta_phi[lower_thetapos-62][lower_phipos-37][k] = pair_delta_t[k];
		//ntuple->Fill(theta, phi, k, pair_delta_t[k]);
	}
          
    // for second pulse, since it is too weak, needs sum of all pair     
	double co_added_max = 0.;
   for(double i_theta=2.5; i_theta<=5.5; i_theta+=0.1){
   		for(double i_phi=-17.1; i_phi<=-16.5; i_phi+=0.1){
