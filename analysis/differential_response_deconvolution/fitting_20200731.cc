...
//int main(int argc, char *argv[]) {
 void fitting(){
 	initial();
 	double measured_delays[4] = {-383.224, -382.120, -381.112, -380.384}; // delays between main and second pulses
 	//double measured_antenna_delays[6] = {-3.1521681, -6.0424853, -47.342842, -2.8903172, -44.190674, -41.300357}; //delays between antennas for main pulse
 	//double measured_antenna_second_delays[6] = {-3.6396090, -7.0978297, -48.734993, -3.4582206, -45.095384, -41.637163}; //delays between antennas for second pulse
 	
 	
 	float _ntuple_theta, _ntuple_phi, _ntuple_pair, _ntuple_delta_t;
 	
 	// _main this one with coadding as second pulse
 	TFile *f_ntuple_main_V = new TFile("_main_V_20200731.root","READ");
	...
	// _main this one with coadding as second pulse
	TFile *f_ntuple_main_H = new TFile("_main_H_20200731.root","READ");
	...
	TFile *f_ntuple_second_H = new TFile("second_H_20200731.root","READ");
	...
	for(int entry=0; entry<ntuple_main_V->GetEntries(); entry++){
		...
		int theta_pos = round((_ntuple_theta-2.5)/0.1);
		int phi_pos = round((_ntuple_phi-(-17.1))/0.6);
		int pair_pos = round(_ntuple_pair);
		delta_t_theta_phi_main_V[theta_pos][phi_pos][pair_pos] = _ntuple_delta_t;
		cout<<theta_pos<<"  "<<_ntuple_theta<<endl;
		cout<<ntuple_main_V->GetEntries()<<endl;
	}
	...
	for(int entry=0; entry<ntuple_second_H->GetEntries(); entry++){
		...
		int theta_pos = round((_ntuple_theta-(-6.5))/0.1);
		int phi_pos = round((_ntuple_phi-(-17.1))/0.6);
		int pair_pos = round(_ntuple_pair);
	...
	TFile *f_out = new TFile("/media/cyc/For_Linux/fitting_result_rough_delay_direct_HV_reflected_20200731.root", "recreate");
	...
	//tr_cr->Branch("reflected_delta_t", _reflected_delta_t, "reflected_delta_t[6]/D");
 	...
 	for(double theta=2.5; theta<=5.5; theta+=0.1){// 3.8 4.8 0.01  2.5 5.5 0.1
 		cout<<"theta   "<<theta<<endl;
 		for(double phi=-17.1; phi<=-16.5; phi+=0.02){// -17.1 -16.5 0.01  -17.1 -16.5 0.02
 			
 			int main_theta_pos = round((theta-2.5)/0.1);
			int main_phi_pos = round((phi-(-17.1))/0.6);
 			...
 			for(double R=10000; R<=500000; R+=5000){//50000 400000 100   10000 500000 100
 				...
 				for(double z=678.2; z<=698.2; z+=0.05){// 678.2 698.2 0.05
 					...
 					for(int ant=0; ant<4; ant++){
 						...
 						chi_square += (pow((delay[ant]-measured_delays[ant])/0.151,2));
 						//cout<<ant<<":   "<<delay[ant]-measured_delays[ant]<<endl;
 					}
 					...
 					for(int pair = 0; pair <6; pair++){
    					...
    					int j_V = j + 4;
							...
							// direct signal
          					...
          					chi_square += (pow((delta_t_V[pair]-delta_t_theta_phi_main_V[main_theta_pos][main_phi_pos][pair])/0.076,2));
          					chi_square += (pow((delta_t_H[pair]-delta_t_theta_phi_main_H[main_theta_pos][main_phi_pos][pair])/0.11,2));
          					...
          					int second_theta_pos = round((reflected_angle[1]-(-6.5))/0.1);//antenna 2
          					if(second_theta_pos<0) second_theta_pos = 0;
							if(second_theta_pos>30) second_theta_pos = 30;
          					chi_square += (pow((delta_t_second_H[pair]-delta_t_theta_phi_second_H[second_theta_pos][main_phi_pos][pair])/0.156,2));
          					//chi_square += (pow((delta_t_second_H[pair]-measured_antenna_second_delays[pair])/0.072,2));
          					//cout<<second_theta_pos<<endl;
          					//cout<<delta_t_theta_phi_second_H[second_theta_pos][main_phi_pos][pair]<<"   "<<measured_antenna_second_delays[pair]<<endl;
