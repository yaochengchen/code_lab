...
for(int high_low=0; high_low<2; high_low++){          
     ...
     if(high_low==0){
     	...
     	upper_phipos = 95;//min(181, m_phipos + 9);//[,)
     	lower_phipos = 93;//max(0, m_phipos - 8);
     	
     	upper_thetapos = 101;// 3 101; -1 61
     	lower_thetapos = 70;// 0 70; -4 30
     ...
     //high elevation events
     	...
     	upper_phipos = min(18, m_phipos + 3);
     	lower_phipos = max(0, m_phipos - 2);
     	
     	upper_thetapos = 10;//min(36, m_thetapos + 6);
...
h2->Draw("colorz");
//h2->GetZaxis()->SetRangeUser(5.5, 6.);
...
/*
  const int NKnownCW = 4;
  double   CWPeak[ NKnownCW ] = {
    187.5/FreqBin, 273.5/FreqBin, 302.5/FreqBin, 313.6/FreqBin  //in cal-pulser, not found in forced trig
...
*/  
  ...
  double   CWPeak[ NKnownCW ] = {
    272.3/FreqBin, 269.5/FreqBin, 267.1/FreqBin, 252.5/FreqBin, 275.3/FreqBin, 277.5/FreqBin, 279.95/FreqBin, 289.8/FreqBin, 271.6/FreqBin, 272.5/FreqBin, 300.15/FreqBin
      //187.5, 272.5 are the strongest
  };  //5.e6, 105.e6, 398.333e6, 483.333e6
...
double a1 = 1. - a0;
 	for(int i=0;i<1500;i++)
		{
		new_x[i] = ((i<Max_i-75)||(i>Max_i+75))?0.:f_x[i];//(a0 - a1*cos(2.*PI*(i-Max_i+200)/(400.-1.)))*f_x[i];
		}
 ...
 */ 
  ...
  const int NKnownCW = 11;
  double   CWPeak[ NKnownCW ] = {
    272.3/FreqBin, 269.5/FreqBin, 267.1/FreqBin, 252.5/FreqBin, 275.3/FreqBin, 277.5/FreqBin, 279.95/FreqBin, 289.8/FreqBin, 271.6/FreqBin, 273.5/FreqBin, 300.15/FreqBin
      //187.5, 272.5 are the strongest
  };  //5.e6, 105.e6, 398.333e6, 483.333e6
...
void low_elevation_reconstruction(int n_thread, int nt_j, double R, int lower_thetapos, int upper_thetapos, int lower_phipos, int upper_phipos, double max_theta[3], double max_phi[3], double max[3]){
	...
	TFile *f_ntuple = new TFile("main_V_20210102.root","RECREATE");
...
get_cross_correlation(n_thread, nt_j);
	...
	for(int k=0; k<6; k++){
		//delta_t_theta_phi[lower_thetapos-70][lower_phipos-93][k] = pair_delta_t[k];
		ntuple->Fill(theta, phi, k, pair_delta_t[k]);
	}
          
    /*      
	double co_added_max = 0.;
   for(double i_theta=-4; i_theta<=-1; i_theta+=0.1){
   		for(double i_phi=-23.2; i_phi<=-22.4; i_phi+=0.1){
   ...
   for(int pair = 0; pair <6; pair++){
    ...
    for(int i_theta=0; i_theta<1; i_theta++){
      for(int i_phi=0; i_phi<6; i_phi++){
          double theta = (thetapos-70)/10.;
          double phi = -0.2 + (0.1)*i_phi + (phipos - 90)*0.6 - 25.;
     ...
     for (int ci=0; ci<5; ci++){
      for (int cj=0; cj<13; cj++){
          ...
          if(inter_map[ci][cj] > max[0])
          {
           ...
           max_phi[2] = max_phi[1]; max_phi[1] = max_phi[0]; max_phi[0] = -0.2 + (0.1)*cj + (phipos - 90)*0.6 -25;
          }
          
          h2->Fill(-0.2 + (0.1)*cj + (phipos - 90)*0.6 -25, (thetapos-70)/10., inter_map[ci][cj]);
     }
   }
