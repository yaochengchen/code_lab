...
for(int high_low=0; high_low<2; high_low++){          
     ...
     if(high_low==0){
     	...
     	upper_phipos = 73;// -24.6
     	lower_phipos = 71;// -25.1
     	
     	upper_thetapos = 125;// -2.5 125; 46
     	lower_thetapos = 94;// -5.5 94; 15
...
double FreqBin = 1250./4096.;
  ...
  const int NKnownCW = 9;
  double   CWPeak[ NKnownCW ] = {
    255.8/FreqBin, 244.2/FreqBin, 260.85/FreqBin, 200/FreqBin, 232.5/FreqBin, 244.2/FreqBin, 249.2/FreqBin, 262.5/FreqBin, 300/FreqBin
  };
...
void low_elevation_reconstruction(int n_thread, int nt_j, double R, int lower_thetapos, int upper_thetapos, int lower_phipos, int upper_phipos, double max_theta[3], double max_phi[3], double max[3]){
	...
	TFile *f_ntuple = new TFile("main_H_20221228.root","RECREATE");
...
get_cross_correlation(n_thread, nt_j);
	for(int k=0; k<6; k++){
		//delta_t_theta_phi[lower_thetapos-15][lower_phipos-71][k] = pair_delta_t[k];
		ntuple->Fill(theta, phi, k, pair_delta_t[k]);
	}

	/*// for second pulse, since it is too weak, needs sum of all pair           
	double co_added_max = 0.;
   for(double i_theta=-5.5; i_theta<=-2.5; i_theta+=0.1){
   		for(double i_phi=-25.1; i_phi<=-24.6; i_phi+=0.1){
