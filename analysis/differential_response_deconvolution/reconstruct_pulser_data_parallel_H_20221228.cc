...
for(int high_low=0; high_low<2; high_low++){          
     ...
     if(high_low==0){
     	...
     	upper_phipos = min(143, m_phipos + 9);//[,)
     ...
     //high elevation events
     	...
     	upper_phipos = 14;//min(15, m_phipos + 3);
     	lower_phipos = 0;//max(0, m_phipos - 2);
     	
     	upper_thetapos = 42;//min(43, m_thetapos + 6);
   ...
   for(int pair = 0; pair <6; pair++){
    ...
    for(int i_theta=0; i_theta<5; i_theta++){
      for(int i_phi=0; i_phi<13; i_phi++){
          ...
          double phi = -0.6 + (0.1)*i_phi + (phipos - 71)*0.6 - 25.;
     ...
     for (int ci=0; ci<5; ci++){
      for (int cj=0; cj<13; cj++){
          ...
          if(inter_map[ci][cj] > max[0])
          {
           ...
           max_phi[2] = max_phi[1]; max_phi[1] = max_phi[0]; max_phi[0] = -0.6 + (0.1)*cj + (phipos - 71)*0.6 -25;
          }
     }
   }
