...
void processing();
void initial();
void draw_candidate_event_coherent_sum_deconvoluted_askaryan(int date, int run_num, int event_num, double in_theta, double in_phi);
...
double cross_correlation_time(double w1[301], double w2[301], double X_cor[101]);
...
void processing(){
	...
	double in_phi;
  	...
  	for(auto filename : filenames){
    	...
    	if (p!=5) break;
    	draw_candidate_event_coherent_sum_deconvoluted_askaryan(date, run_num, event_num, in_theta, in_phi);
    	}
...
void draw_candidate_event_coherent_sum_deconvoluted_askaryan(int date, int run_num, int event_num, double in_theta, double in_phi){
  ...
  for (int i=0;i<15000;i++)
   ...
   for(int ant = 0; ant < 4; ant++){
    ...
    RMS_theta = sqrt(RMS_theta/(9000.-801.));
    RMS_phi = sqrt(RMS_phi/(9000.-801.));
    ...
    FW_waveform1->GetXaxis()->SetRangeUser(150,350);
    ...
    FW_waveform2->GetXaxis()->SetRangeUser(150,350);
    ...
    string fig_name = "./photos/" + std::to_string(date) + "_waveform_" + std::to_string(run_num) + "_" + std::to_string(event_num) + "_cyc_label_filter" + ".png";
...
double theta = in_theta*TMath::DegToRad();;
double phi = tempt_phi*TMath::DegToRad();;

//double B_field[3] = {-2957.5, 36320.7, -26689.5};

//rotate to antenna coordinate
double B_field[3] = {-18208.5, 32963.6, -26689.5};

double B_scale = sqrt(B_field[0]*B_field[0] + B_field[1]*B_field[1] + B_field[2]*B_field[2]);



B_field[0] = B_field[0]/B_scale;
B_field[1] = B_field[1]/B_scale;
B_field[2] = B_field[2]/B_scale;

double n_comein[3] = {0.};
//-1 for come in signal
...
n_phi[0] = TMath::Cos(phi+90*TMath::DegToRad());
n_phi[1] = TMath::Sin(phi+90*TMath::DegToRad());
n_phi[2] = 0;

//see definition of HFSS antenna simulation
n_theta[0] = TMath::Cos(theta-90*TMath::DegToRad())*TMath::Cos(phi);
n_theta[1] = TMath::Cos(theta-90*TMath::DegToRad())*TMath::Sin(phi);
n_theta[2] = TMath::Sin(theta-90*TMath::DegToRad());

double P_geomag_emission[3] = {0};
double V_geomag_emission[3] = {0};

cross_product(n_comein, B_field, P_geomag_emission);

double P_scale = sqrt(P_geomag_emission[0]*P_geomag_emission[0] + P_geomag_emission[1]*P_geomag_emission[1] + P_geomag_emission[2]*P_geomag_emission[2]);

P_geomag_emission[0] = P_geomag_emission[0]/P_scale;
P_geomag_emission[1] = P_geomag_emission[1]/P_scale;
P_geomag_emission[2] = P_geomag_emission[2]/P_scale;

cross_product(n_comein, P_geomag_emission, V_geomag_emission);

double V_scale = sqrt(V_geomag_emission[0]*V_geomag_emission[0] + V_geomag_emission[1]*V_geomag_emission[1] + V_geomag_emission[2]*V_geomag_emission[2]);

cout<<V_scale<<endl;

double E_field[15000][3] = {0};
double E_P_geomag[15000] = {0};
double E_V_geomag[15000] = {0};
...
for(int i=0; i<15000; i++){
	//added_E_theta[i] = 0.7;
	//added_E_phi[i] = 0.3;
	E_field[i][0] = added_E_theta[i] * n_theta[0] + added_E_phi[i] * n_phi[0];
	E_field[i][1] = added_E_theta[i] * n_theta[1] + added_E_phi[i] * n_phi[1];
	E_field[i][2] = added_E_theta[i] * n_theta[2] + added_E_phi[i] * n_phi[2];
	//cout<<E_field[i][0]<<"  "<<E_field[i][1]<<"  "<<E_field[i][2]<<"  "<<endl;
	E_P_geomag[i] = dot_product(E_field[i], P_geomag_emission);
	E_V_geomag[i] = dot_product(E_field[i], V_geomag_emission);
	//cout<<E_P_geomag[i]<<"  "<<E_V_geomag[i]<<endl;
	if(fabs(E_P_geomag[i]) > max){
		max = fabs(E_P_geomag[i]);
		max_i = i;
	}
	
	//E_field[i][0] = E_P_geomag[i] * P_geomag_emission[0] + E_V_geomag[i] * V_geomag_emission[0];
	//E_field[i][1] = E_P_geomag[i] * P_geomag_emission[1] + E_V_geomag[i] * V_geomag_emission[1];
	//E_field[i][2] = E_P_geomag[i] * P_geomag_emission[2] + E_V_geomag[i] * V_geomag_emission[2];

	//E_P_geomag[i] = dot_product(E_field[i], n_theta);
	//E_V_geomag[i] = dot_product(E_field[i], n_phi);
		
}

	double X_cor[101] = {0};
	double X_t[101] = {0};
	for(int i=0;i<101;i++){X_t[i]=(i-50)*0.08;}
	double askaryan_delay = cross_correlation_time(&E_P_geomag[max_i-150], &E_V_geomag[max_i-150], X_cor);


	TCanvas *c4 = new TCanvas("c4","Canvas Example",1500,1000);

    TGraph *FW_waveform3 = new TGraph(15000, f_t, E_P_geomag);
    FW_waveform3->Draw("APL");
    FW_waveform3->GetXaxis()->SetRangeUser(150,350);
    FW_waveform3->GetXaxis()->SetTitle("ns");
    FW_waveform3->GetYaxis()->SetTitle("mV/m");
    //string title_name = std::to_string(theta_phi_ratio) + "+-" + std::to_string(ratio_error);
    //FW_waveform3->SetTitle(title_name.c_str());
    FW_waveform3->SetLineColor(2);

    //c3->cd(2);
    TGraph *FW_waveform4 = new TGraph(15000, f_t, E_V_geomag);
    FW_waveform4->Draw("PL same");
    FW_waveform4->GetXaxis()->SetRangeUser(150,350);
    FW_waveform4->GetXaxis()->SetTitle("ns");
    FW_waveform4->GetYaxis()->SetTitle("mV/m");
    //FW_waveform4->SetTitle(title_name.c_str());
    FW_waveform4->SetLineColor(4);


	string fig_name_4 = "./photos/" + std::to_string(date) + "_geo_waveform_" + std::to_string(run_num) + "_" + std::to_string(event_num) + "_cyc_label_filter_" + std::to_string(askaryan_delay) + "_ns" + ".png";
    c4->SaveAs(fig_name_4.c_str());

	TCanvas *c5 = new TCanvas("c4","Canvas Example",1500,1000);
	c5->cd();
	TGraph *X_cor_waveform = new TGraph(101, X_t, X_cor);
	X_cor_waveform->Draw("APL");
	X_cor_waveform->GetYaxis()->SetRangeUser(-1,1);
	string fig_name_5 = "./photos/" + std::to_string(date) + "_X_cor_" + std::to_string(run_num) + "_" + std::to_string(event_num) + "_cyc_label_filter_" + std::to_string(askaryan_delay) + "_ns" + ".png";
    c5->SaveAs(fig_name_5.c_str());
...
double cross_correlation_time(double w1[301], double w2[301], double X_cor[101]){
	...
	for(int j =0; j<101; j++){
		...
		for(int i =0; i<201; i++){
			int step1 = i+50;
			int step2 = i+j;
			factor1 += w1[step1]*w1[step1];
			factor2 += w2[step2]*w2[step2];
		}
...
cout<<(max_j-50)*0.08<<endl;
return (max_j-50)*0.08;
