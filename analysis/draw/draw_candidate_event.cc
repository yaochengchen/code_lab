...
void draw_candidate_event(int date, int run_num, int event_num){
 ...
 //read_responses_high();
	...
	/*
	// caution! T1V~ should change from Short_t To Int_t
  ...
  for (int i=0;i<1500;i++)
    ...
    c1->cd(channel+1);
    TGraph *original_waveform = new TGraph(1500, t, x[channel]);
    original_waveform->Draw("APL");
    original_waveform->SetLineColor(channel/2+1);

string c1_fig_name = "./photos/" + std::to_string(date) + "_original_waveform_" + std::to_string(run_num) + "_" + std::to_string(event_num) + "_" + ".png";
    c1->SaveAs(c1_fig_name.c_str());
 
    
    double FW_power_spectrum[8][751] = {0.};
    double FW_frequency[751] = {0.};
    double T = 1200e-9;
    double n_size = 1500.;
    for(int k=0; k<751; k++){
    	FW_frequency[k] = k*5./6.;
    ...
    TGraph *FW_waveform = new TGraph(1500, t, deep_filtered_x[channel]);
    FW_waveform->Draw("APL");
    
    FW_waveform->GetXaxis()->SetTitle("ns");
    FW_waveform->GetYaxis()->SetTitle("mV");
    FW_waveform->GetXaxis()->SetRangeUser(250,750);
    if(channel%2==0){
    	string channel_name = "T"+to_string(channel/2+1)+"H";
    	FW_waveform->SetTitle(channel_name.c_str());
    	FW_waveform->GetYaxis()->SetRangeUser(-100,100);
    }
    if(channel%2==1){
    	string channel_name = "T"+to_string(channel/2)+"V";
    	FW_waveform->SetTitle(channel_name.c_str());
    	FW_waveform->GetYaxis()->SetRangeUser(-100,100);
    }
    FW_waveform->SetLineColor(channel/2+1);
    ...
    c2->cd(channel+1);
    TGraph *FW_spectrum = new TGraph(751, FW_frequency, FW_power_spectrum[channel]);
    ...
    //c2->SaveAs("./FW_spectrum.png");
    

    //HALF_WINDOW_WIDTH
    double SW_power_spectrum[8][751] = {0.};
    double SW_frequency[751] = {0.};
    T = 120e-9;
    n_size = 150.;
    //HALF_WINDOW_WIDTH
    for(int k=0; k<HALF_WINDOW_WIDTH; k++){
    	SW_frequency[k] = k*50./6.;
    	double power = 2.*(T*1.0e06)*(SW_re[channel][k]*SW_re[channel][k]+SW_im[channel][k]*SW_im[channel][k])/(n_size*n_size*50.*1000.);
    	//cout<<power<<endl;
    	if(power<=0){power = 1.0e-12;}
    	SW_power_spectrum[channel][k] = 10*log10(power);
    	//cout<<SW_power_spectrum[k]<<endl;
    	}
    ...
    //cout<<"here"<<endl;
    //HALF_WINDOW_WIDTH
    TGraph *SW_spectrum = new TGraph(76, SW_frequency, SW_power_spectrum[channel]);
    //SW_spectrum->Draw("PL same");
    SW_spectrum->SetLineColor(2);
    ...
    TGraph *filtered_waveform = new TGraph(1500, t, SW_x[channel]);
    //filtered_waveform->Draw("APL");

    //c5->SaveAs("./SW_waveform.png");
...
string fig_name = "./photos/" + std::to_string(date) + "_filtered_waveform_" + std::to_string(run_num) + "_" + std::to_string(event_num) + "_" + ".png";
    c3->SaveAs(fig_name.c_str());
    //c2->SaveAs("./SW_spectrum.png");
...
int get_FFT(int ch, double x[1500], double FW_x[1500], double filtered_x[1500], double FW_re[751], double FW_im[751], double deep_filtered_x[1500]){
   ...
   fft_back_x = filter_fft_back->GetPointsReal();
	...
	for(int i=0; i<1500; i++){data[i]=x[i];}
