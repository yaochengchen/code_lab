...
void Planck_taper_windowing(double epsilon, int N, double x[], double w_x[]);
 ...
 double ant_re[8][751] = {0};
 double ant_im[8][751] = {0};
...
double Correlator_re[2][6][751] = {0};            
double Correlator_im[2][6][751] = {0};
...
cout<<endl;
    ...
    //fname_evt = "/home/cyc/software/TAROGE-4_analysis/make_pulser_data/data/make_coreas_data_with_pps_real1775.000000.root";
    //fname_evt = "/media/cyc/For_Linux/CoREAS_Sim/proton/cyc/Coreas-t4-r000001.root";//"/home/cyc/software/TAROGE-4_analysis/make_pulser_data/data/make_coreas_data_with_pps1875.000000.root";

	TFile *file = new TFile(fname_evt);
    TTree *Tree_Muon = (TTree*) file->Get("t");
    //TTree *Tree_Muon = (TTree*) file->Get("EfieldTree");
  ...
  for (int i=0;i<1500;i++)
	...
	int pol = 1;
	double masked_band = filtering(pol);
...
get_cross_correlation(pol, tdoa, Xcor);
reconstruction(pol, event_theta, event_phi, x_value);
    
gStyle->SetPalette(55);
h2->Draw("colorz");
//h2->GetZaxis()->SetRangeUser(3, 3.4);
...
// for second pulse 
		...
		for(int i=300;i<1000;i++){
			if(i>Max_i+50){
				if(fabs(filtered_x[ch][i])>s_Max) {s_Max = fabs(filtered_x[ch][i]);  s_Max_i = i;}
			}
  		}
  		Max_i = s_Max_i;
*/  						
...
void get_cross_correlation(int pol, double tdoa[2][6], double Xcor[2][6]){	
   ...
   for(int pair = 0; pair <6; pair++){
	...
	//cout<<"pair:  "<<pair<<"  "<<correlation_pair[pol][0][103]<<endl;
...
//cout<<"final:  "<<correlation_pair[pol][0][103]<<endl;
}
...
//if((pol==0)&&(pair==5)) cout<<Correlator_time[0]<<endl;
         ...
         int mm;
				...
				//if(correlation_pair[pol][pair][k]>100)cout<<"fuck"<<endl;
                                                    }//k 
                //cout<<"here:  "<<pair<<"  "<<correlation_pair[pol][0][103]<<endl;
