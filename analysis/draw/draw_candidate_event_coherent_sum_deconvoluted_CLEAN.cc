...
void processing();
void initial();
void draw_candidate_event_coherent_sum_deconvoluted_CLEAN(int date, int run_num, int event_num, double in_theta, double in_phi, int polarity[2], int filter);
...
void CLEAN_deconvolution(double Re_ant[8][2049], double Im_ant[8][2049], double ANT_V_theta_re[2049], double ANT_V_theta_im[2049], double ANT_H_theta_re[2049], double ANT_H_theta_im[2049], double ANT_H_phi_re[2049], double ANT_H_phi_im[2049], bool label[2049], double **E_theta, double **E_phi, double ws_dec[409600], int polarity[2], int filter);
void CLEAN_FFT(double x[409600], double *re, double *im, int filter);
...
void CLEAN_convolution(double *re, double *im, double ANT_H_theta_re[2049], double ANT_H_theta_im[2049], int channel);
...
void Adjust_delay(double Re_ant[8][2049], double Im_ant[8][2049], double H_V_delay, double in_theta, double in_phi);
  ...
  int N_small = 4096;
  TVirtualFFT *fft_back_small = TVirtualFFT::FFT(1, &N_small, "C2RBACKWARD M K"); 
...
void draw_candidate_event_coherent_sum_deconvoluted_CLEAN(int date, int run_num, int event_num, double in_theta, double in_phi, int polarity[2], int filter){
  ...
  for (int i=0;i<1500;i++)
	...
	*/
	...
	double ** E_phi = new_Array2D<double>(4, 409600);
	double ** E_theta = new_Array2D<double>(4, 409600);
	...
	CLEAN_deconvolution(Re_ant, Im_ant, ANT_V_theta_re[i_theta][i_phi], ANT_V_theta_im[i_theta][i_phi], ANT_H_theta_re[i_theta][i_phi], ANT_H_theta_im[i_theta][i_phi], ANT_H_phi_re[i_theta][i_phi], ANT_H_phi_im[i_theta][i_phi], label, E_theta, E_phi, ws_dec, polarity, filter);
	...
	double *clean_time = new double[409600];
	for(int i=0; i<409600; i++){
		clean_time[i] = 0.008*i;
	
	}
   
   for(int ant = 0; ant < 4; ant++){
   		TGraph *cyc = new TGraph(409600, clean_time, E_theta[ant]);
   		TGraph *wyn = new TGraph(409600, clean_time, E_phi[ant]);
   		c1->cd(2*ant+1);
   		cyc->Draw("AL");
    ...
    double *added_E_theta = new double[409600];
    double *added_E_phi = new double[409600];
	for(int i=0; i<409600; i++){added_E_theta[i] = 0; added_E_phi[i] = 0;}
    ...
    RMS_theta = sqrt(RMS_theta/(409600.-801.));
    RMS_phi = sqrt(RMS_phi/(409600.-801.));
    ...
    TGraph *FW_waveform1 = new TGraph(409600, clean_time, added_E_theta);
    ...
    //c3->cd(2);
    TGraph *FW_waveform2 = new TGraph(409600, clean_time, added_E_phi);
    ...
    FW_waveform2->Draw("AL");
    FW_waveform1->Draw("L same");
...
void get_FFT(int channel, bool label[2049]){
    ...
    fft_back_small->SetPointsComplex(Re_ant[channel], Im_ant[channel]);
    fft_back_small->Transform();
    Correlator_time = fft_back_small->GetPointsReal();
...
void CLEAN_deconvolution(double Re_ant[8][2049], double Im_ant[8][2049], double ANT_V_theta_re[2049], double ANT_V_theta_im[2049], double ANT_H_theta_re[2049], double ANT_H_theta_im[2049], double ANT_H_phi_re[2049], double ANT_H_phi_im[2049], bool label[2049], double **E_theta, double **E_phi, double ws_dec[409600], int polarity[2], int filter){
	...
	double *re = new double[204801];
	double *im = new double[204801];
	...
	for(int pol=1; pol>=0; pol--){
		...
		if(pol==0){CLEAN_convolution(re, im, ANT_H_theta_re, ANT_H_theta_im, channel);}// H-pol minus E_theta 
   		...
   		for(Int_t k=0; k<2049; k++){// 0 means DC component // from 185-350 MHz
       		...
       		if(pol==0){Re_ant[channel][k] -= re[k]; Im_ant[channel][k] -= im[k];}// H-pol minus E_theta 
       		
       		TComplex ANT_;
       		if(pol==1) {TComplex ANT_V(ANT_V_theta_re[k], ANT_V_theta_im[k]); ANT_ = ANT_V;}
       		else{TComplex ANT_H(ANT_H_phi_re[k], ANT_H_phi_im[k]); ANT_ = ANT_H;}
       		TComplex FEE(FEE_re[channel][k], FEE_im[channel][k]);
       		TComplex response = ANT_*FEE;//caution!!! cyc
       		
       		response_re[k] = response.Re()/100. * (!label[k]); // from FFT 4096 to IFFT 409600, impulse response should reduce
       		response_im[k] = response.Im()/100. * (!label[k]);
       	}
       	
       	double tdoa_best = 0.;
       	//double ws_xcor[409600];
       	double *ws_xcor = new double[409600];
		for(int i=0; i<409600; i++){ws_xcor[i] = 0; ws_dec[i]=0;}
       	double tmp_n_peak;
       	
       	double xcorPeak = FFTCrossCorrelate(Re_ant[channel], Im_ant[channel], response_re, response_im, ws_xcor, tdoa_best, tmp_n_peak, 0);
       	
		const double Threshold = 0.05 * fabs(xcorPeak); //#### of peak xcor coeff
		
		//double ws_hh_xcor[409600];
		double *ws_hh_xcor = new double[409600];
		for(int i=0; i<409600; i++){ws_hh_xcor[i] = 0;}
		
		double tmp_tdoa;
		double n_peak_hh;
		const double Norm_hh = FFTCrossCorrelate(response_re, response_im, response_re, response_im, ws_hh_xcor, tmp_tdoa, n_peak_hh, 1);
		
		cout<<ant<<"  "<<xcorPeak<<"   "<<Norm_hh<<"    "<<tdoa_best<<"    "<<tmp_tdoa<<"    "<<tmp_n_peak<<"  "<<n_peak_hh<<endl;
		...
		{	
			int n_peak_hy = 0;
			double max_xcorPeak = TMath::MaxElement(409600, ws_xcor);
			double min_xcorPeak = TMath::MinElement(409600, ws_xcor);
			if(step==0){
				if(polarity[pol]==1){xcorPeak = max_xcorPeak; n_peak_hy = TMath::LocMax(409600, ws_xcor);}
				else if(polarity[pol]==-1){xcorPeak = min_xcorPeak; n_peak_hy = TMath::LocMin(409600, ws_xcor);}
				else if(polarity[pol]==0){
					if(max_xcorPeak>fabs(min_xcorPeak)){
						xcorPeak = max_xcorPeak; n_peak_hy = TMath::LocMax(409600,ws_xcor);
					}
					else{
						xcorPeak = min_xcorPeak; n_peak_hy = TMath::LocMin(409600, ws_xcor);
					}
				}
			...
			const int N_max = 409600;//min( 409600, 409600 + n_shift ); //the shorter one
			for(int n=N_min; n< N_max; n++)
			{
				int i = n - n_shift;
				if(i<0) i += 409600;
				if(i>=409600) i -= 409600;
				//cout<<i<<"  "<<n<<endl;
			...
			else{ws_dec[ 409600 - n_shift] += scale;}
			...
			c1->Divide(2,1);
			c1->cd(1);
			double *t = new double[409600];
			for(int i=0; i<409600; i++){t[i] = i*0.008;}
			TGraph *cyc = new TGraph(409600, t, ws_xcor);
			cyc->Draw();
			c1->cd(2);
			TGraph *wyn = new TGraph(150000, t, ws_hh_xcor);
			wyn->Draw();
			...
			*/
			...
			double *t = new double[409600];
			for(int i=0; i<409600; i++){t[i] = i*0.008;}
			TGraph *cyc = new TGraph(409600, t, ws_dec);
			cyc->Draw();
		
		
     	
     		CLEAN_FFT(ws_dec, re, im, filter);
     		

     		
     	
     		c1->cd(2);
			TGraph *wyn = new TGraph(409600, t, ws_dec);
			wyn->Draw();
			string c1_fig_name = "./photos/" + std::to_string(channel) + "_CLEAN_waveform_" + std::to_string(step) + ".png";
    		c1->SaveAs(c1_fig_name.c_str());
     	
     	for(int i=0; i<409600; i++){
     		if(pol==1){E_theta[ant][i] = ws_dec[i];}
     		else{E_phi[ant][i] = ws_dec[i];}
...
double FFTCrossCorrelate(double ws1_re[2049], double ws1_im[2049], double ws2_re[2049], double ws2_im[2049], double ws_xcor[409600], double &tdoa_best, double &n_peak, int polarity){	
        ...
        fft_back_small->SetPointsComplex(Correlator_re,Correlator_im);
        fft_back_small->Transform();
        Correlator_time = fft_back_small->GetPointsReal();
        ...
        */
        ...
        for(Int_t k=0; k<409600; k++){
        	int i = 75000+k;
        	if(i>=409600){i-=409600;}
        	ws_xcor[i] = Correlator_time[k]/(sqrt(sum_1*sum_2)*409600.);
        }
...
void CLEAN_FFT(double x[409600], double *re, double *im, int filter){
	...
	for(int i=0; i<204801; i++){re[i] = 0; im[i] = 0;}
	fft_forward->SetPoints(x);
   	fft_forward->Transform();
   	fft_forward->GetPointsComplex(re,im);
   	
   	for (Int_t k=0; k<204801; k++){//0 means DC component // from 185-350 MHz
       //if((k<606)||(k>1147)) {re[k] = 0.; im[k] = 0.;}
       if((k>2049)) {re[k] = 0.; im[k] = 0.;}
    }
...
void CLEAN_convolution(double *re, double *im, double ANT_H_theta_re[2049], double ANT_H_theta_im[2049], int channel){
	...
	for (Int_t k=0; k<204801; k++){//0 means DC component // from 185-350 MHz
   		...
   		TComplex ANT_H(ANT_H_theta_re[k], ANT_H_theta_im[k]);
  		TComplex FEE(FEE_re[channel][k], FEE_im[channel][k]);
  		TComplex response = FEE*ANT_H;
  		double response_re = response.Re();
  		double response_im = response.Im();
  		
       if((k<606)||(k>1147)) {re[k] = 0.; im[k] = 0.;}
       else{
       		TComplex x(re[k], im[k]);
       		TComplex y(response_re, response_im);
       		TComplex z = x*y;
       		re[k] = z.Re();
       		im[k] = z.Im();
       }
    ...
    for(Int_t k=0; k<4096; k++){
        	x[k] = Correlator_time[k*100]/409600.;
    }
...
void CLEAN_simulation(double x[4096], double E_field_x[4096], double ANT_V_theta_re[2049], double ANT_V_theta_im[2049], int channel){
    ...
    fft_back_small->SetPointsComplex(re,im);
    fft_back_small->Transform();
    Correlator_time = fft_back_small->GetPointsReal();
