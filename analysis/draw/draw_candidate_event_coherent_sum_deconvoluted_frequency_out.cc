...
void Deconvolute_antenna(double Re_ant[8][2049], double Im_ant[8][2049], double ANT_H_theta_re[2049], double ANT_H_theta_im[2049], double ANT_H_phi_re[2049], double ANT_H_phi_im[2049], double ANT_V_theta_re[2049], double ANT_V_theta_im[2049], double ANT_V_phi_re[2049], double ANT_V_phi_im[2049], double E_theta[4][4096], double E_phi[4][4096], double E_theta_re_all[2049], double E_theta_im_all[2049], double E_phi_re_all[2049], double E_phi_im_all[2049]);
...
void draw_candidate_event_coherent_sum_deconvoluted(int date, int run_num, int event_num, double in_theta, double in_phi){
	...
	/*
	// caution! T1V~ should change from Short_t to Int_t  and H_V delay should set to 0
	
	//int run = 34009;
	const string coreas_root = Form("/media/cyc/For_Linux/CoREAS_Sim/proton/cyc/Coreas-t4-r%06d.root", run_num);

	TFile *coreas_file = new TFile(coreas_root.c_str());
	
	TTree *EfieldTree = (TTree*) coreas_file->Get("EfieldTree");

	EfieldTree->SetBranchAddress("T1V", T1V);
	EfieldTree->SetBranchAddress("T2V", T2V);
	EfieldTree->SetBranchAddress("T3V", T3V);
	EfieldTree->SetBranchAddress("T4V", T4V);
	EfieldTree->SetBranchAddress("T1H", T1H);
	EfieldTree->SetBranchAddress("T2H", T2H);
	EfieldTree->SetBranchAddress("T3H", T3H);
	EfieldTree->SetBranchAddress("T4H", T4H);
	
	EfieldTree->GetEntry(event_num);
	*/
  ...
  for (int i=0;i<15000;i++)
	...
	//i_theta = 18;
	//i_phi = 0;
	double E_theta_re_all[2049] = {0.};
   	double E_theta_im_all[2049] = {0.};
   	double E_phi_re_all[2049] = {0.};
   	double E_phi_im_all[2049] = {0.};
	Deconvolute_antenna(Re_ant, Im_ant, ANT_H_theta_re[i_theta][i_phi], ANT_H_theta_im[i_theta][i_phi], ANT_H_phi_re[i_theta][i_phi], ANT_H_phi_im[i_theta][i_phi], ANT_V_theta_re[i_theta][i_phi], ANT_V_theta_im[i_theta][i_phi], ANT_V_phi_re[i_theta][i_phi], ANT_V_phi_im[i_theta][i_phi], E_theta, E_phi, E_theta_re_all, E_theta_im_all, E_phi_re_all, E_phi_im_all);
   ...
   for(int ant = 0; ant < 4; ant++){
   		TGraph *cyc = new TGraph(1500, t, E_theta[ant]);
   		TGraph *wyn = new TGraph(1500, t, E_phi[ant]);
   		c1->cd(ant+1);
   		cyc->Draw("APL");
   		cyc->SetLineColor(ant+1);
   		//c1->cd(2*ant+2);
   		wyn->Draw("PL same");
   		wyn->SetLineColor(ant+5);
   		
   		TSpline3 *s_theta = new TSpline3("grs", t, E_theta[ant], 1500);
   		TSpline3 *s_phi = new TSpline3("grs", t, E_phi[ant], 1500);
   		for(int i=0; i<15000; i++){
   			_E_theta[ant][i] = s_theta->Eval(0.08*i- 0* 0.008*delay_theta[ant]);
   			_E_phi[ant][i] = s_phi->Eval(0.08*i- 0* 0.008*delay_phi[ant]);
   			}
   		delete s_theta;
   		s_theta = nullptr;
   		delete s_phi;
    ...
    double rough_added_E_theta[15000] = {0.};
    double rough_added_E_phi[15000] = {0.};
    ...
    for(int ant=0; ant<4; ant++){
    	...
    	for(int i=0; i<1500; i++){
    		rough_added_E_theta[i] += E_theta[ant][i]/4.;
    		rough_added_E_phi[i] += E_phi[ant][i]/4.;
    	}
    }
    
    const char * output_file=  Form("/home/cyc/software/TAROGE-4_analysis/CR_candidates/%d_run%08d_evnet%05d.txt", date, run_num, event_num);
    fstream out_waveform;
    out_waveform.open(output_file, ios::out | ios::trunc);// without ios::out   clean the file before write
    
    for(Int_t k=0; k<2049; k++){//0 means DC component // from 185-350 MHz
    	out_waveform<<E_theta_re_all[k]<<"   "<<E_theta_im_all[k]<<"   "<<E_phi_re_all[k]<<"   "<<E_phi_im_all[k]<<"   ";
       	if((k<606)||(k>1147)){out_waveform<<0<<endl;}
       	else{out_waveform<<((int) (!label[k]))<<endl;}
    }
       		
       		
    for(int i=0; i<15000; i++){
    	out_waveform<<added_E_theta[i]<<"   "<<added_E_phi[i]<<endl;
    }
	...
	cout<<"max_E_theta_i: "<<max_E_theta_i<<"  max_E_phi_i:  "<<max_E_phi_i<<endl;
	...
	//TGraph *rough_FW_waveform1 = new TGraph(1500, t, rough_added_E_theta);
	//TGraph *rough_FW_waveform2 = new TGraph(1500, t, rough_added_E_phi);
	//rough_FW_waveform1->Draw("PL same");
	//rough_FW_waveform2->Draw("PL same");
   ...
   for(int ant=0; ant<4; ant++){
   		...
   		for(Int_t k=0; k<2049; k++){//0 means DC component // from 185-350 MHz
       		...
       		E_phi_im[ant][k] = Im_ant[2*ant][k];
       			...
       			E_theta_re_all[k] += E_theta_re[ant][k]/4.;
       			E_theta_im_all[k] += E_theta_im[ant][k]/4.;
       			E_phi_re_all[k] += E_phi_re[ant][k]/4.;
       			E_phi_im_all[k] += E_phi_im[ant][k]/4.;
     ...
     for(int i=-1000; i<1000; i+=10){
     	for(int j=-1000; j<1000; j+=10){
     		for(int k=-1000; k<1000; k+=10){
     			...
     			if(fabs(sum_theta)>max_theta){
     			max_theta=fabs(sum_theta);delay_theta[1]=i;delay_theta[2]=j;delay_theta[3]=k;
     			}
     			if(fabs(sum_phi)>max_phi){
     			max_phi=fabs(sum_phi);delay_phi[1]=i;delay_phi[2]=j;delay_phi[3]=k;
     			}
     		}
     	}
     }
