...
void get_tdoa(int pol, double gTDOA[6], double tdoa[6]); 
...
int find_CW(int ch, bool label[751]);
int filter_fre(int ch, bool label[751], double &SNR, int &multi_pulses);
int get_FFT(int ch);
...
int filtering(int pol, double saSNR[4], int multi_pulses[4]);
...
double spectrum_cut(int pol, int run_pos, double V_peak);
...
Double_t pps_ave_power[200][8][20] = {0.};//199 //180-350MHz in 199 bins; one day maximum has 172 runs
Double_t power_frequency[20] = {0.};//180-350MHz in 199 bins;
Double_t trigger_spectrum[8][20] = {0.};
Double_t band_trigger_threshold[8][20] = {0.};

bool label[751] = {false};
...
double rough_SNR[4] = {0.};
double pulse_power[4] = {0.};
double gTDOA[6] = {0.};
double reconstructed_theta[2] = {0.};
double reconstructed_phi[2] = {0.};
int multi_pulses[4] = {0};
int masked_band = 0;
...
#ifdef SIMULATION
	double ShowerTheta, ShowerPhi;
#endif
TBits* triggerBitsccc = 0;
TBits* gOverVoltBits = 0;
...
out_filename =  Form("/media/cyc/SSD_data_disk/from_Data_disk/Linux/TAROGE4_PROCESSED_DATA/240822_t_selection_%08d.root", which_date);
...
for(int i=0; i<1500; i++){t[i] = 0.8*i;}
...
nt_j=0;
		  ...
		  #endif
		  #ifdef SIMULATION
		  		tr_event->SetBranchAddress("ShowerTheta", &ShowerTheta);
		  		tr_event->SetBranchAddress("ShowerPhi", &ShowerPhi);
		  #endif
		  ...
		  tr_event->SetBranchAddress("overVoltBits", &gOverVoltBits);
 ...
 pass_count[0] = event_number;
	...
	for(int entry = 0; entry<event_number; entry++){
		...
		if(gOverVoltBits->CountBits() != 0) {continue;}
  ...
  if(Is_saturated()) {continue;}
		...
		for(int ch=0; ch<8; ch++){
			...
			for(int k=0; k<20; k++){
				...
				for(int l=-5; l<6; l++){
					...
					//if(power != 0){// 20230829 cyc
						average_power += power;
						count++;
					//}
				}
				//cout<<"count:  "<<count<<"   "<<average_power<<endl;
				if(count!=0){
				average_power = average_power/((double) count);
				// in dB unit
				//pps_ave_power[run_pos][ch][k] += average_power;
				//pps_ave_power[run_pos][ch][k] += 10*log10(average_power);
				//cout<<num_pps<<endl;
				median[ch][k][num_pps] = average_power;
				//trigger_spectrum[ch][k] = 10*log10(average_power);
				}
				// take average in dB unit, average_power = 1; 
				// take average in normal unit, average_power = 0;
				else{
				//average_power = 0;
				//pps_ave_power[run_pos][ch][k] += average_power;
				//pps_ave_power[run_pos][ch][k] = -1000;
				median[ch][k][num_pps] = 0;
			...
			//cout<<Re_ant[ch][322]<<"   "<<Im_ant[ch][322]<<endl;
			//cout<<pps_ave_power[run_pos][ch][100]<<endl;
		}
	//gRunNum = run_pos;
	//selectionTree->Fill();
	num_pps++;
    ...
    for(int ch=0; ch<8; ch++){
    	for(int k=0; k<20; k++){
    		band_trigger_threshold[ch][k] = pps_ave_power[run_pos][ch][k];
    	}
    }
//60 70
for(int entry = 0; entry<event_number; entry++){
...
if(entry==0) eventTime_start = eventTime;
  ...
  for (int i=0;i<1500;i++)
	...
	//cout<<x[0][502]<<"  "<<x[1][502]<<endl;
	#ifndef SIMULATION
		  if(Is_saturated()) {cout<<"saturated!!"<<endl; continue;}
	#endif
...
int mul_pulses[4] = {0};
	...
	// set default to be V-pol, use V-pol to calculat behavious of pps data
	int pol = 1;
	if(H_trigger_bit>V_trigger_bit){pol = 0;}

	masked_band = filtering(pol, saSNR, mul_pulses);
	//cout<<"masked_band: "<<masked_band<<endl;

	for(int ant=0; ant<4; ant++){
		rough_SNR[ant] = saSNR[ant];
		multi_pulses[ant] = mul_pulses[ant];
		pulse_power[ant] = Sum_x[2*ant+pol];
		}
	...
	//for(int pol=0; pol<2; pol++){
		...
		if(true){
    		...
    		pass_trigger_ratio[pol] = spectrum_cut(pol, run_pos, average_peak);
    		

    		//cout<<pass_trigger_ratio[pol]<<endl;
				
    		//if(pass_trigger_ratio[pol]>0.5){
    		if(true){
				get_cross_correlation(pol, nt_j, tdoa, Xcor);
				
				get_tdoa(pol, gTDOA, tdoa[pol]);
				
				reconstruction(pol, event_theta, event_phi, x_value);
				reconstructed_theta[pol] = event_theta[pol];
				reconstructed_phi[pol] = event_phi[pol];
				//cout<<x_value[nt_j][pol]<<endl;
					
				reconstruct_cross_correlation[pol] = x_value[pol]/6.;
				
				
				//if(x_value[nt_j][pol]>3.6){
				if(true){
					double multi_pulse_cross_correlator = multi_pulse_cross_correlation(pol, event_theta[pol], event_phi[pol]);
					...
					//if(true){
						// can not do this since filtered_x for other pol is all 0;
						//double cross_pol_cross_correlator = cross_pol_cross_correlation(pol, event_theta[pol], event_phi[pol]);
...
int filtering(int pol, double saSNR[4], int multi_pulses[4]){
	...
	int masked_band = std::count(label, &label[750], true);
	
	for(int ant = 0; ant < 4; ant ++){
		int ch = 2*ant + pol;
		Sum_x[ch] = 0;
	
    	int max_i = filter_fre(ch, label, sSNR[ant], sMulti_pulses[ant]);
    	peak_position[ch] = max_i;
    	//cout<<max_i<<endl;
    	//max_i = 750;// caution cyc
    	filter_time(filtered_x[ch], new_x[ch], max_i);

    //for(int j=0;j<1500;j++){// make waveform to be 100 ns
      //if((j<(max_i-80)) || (j>(max_i+80)))new_x[ant][j] = 0.;
    //}    	

   sXcor_fft_forward->SetPoints(&new_x[ch][max_i-HALF_WINDOW_SPACING]);
   sXcor_fft_forward->Transform();
   sXcor_fft_forward->GetPointsComplex(ant_re[ch],ant_im[ch]);

    	  for(int i = 0; i <WINDOW_SPACING; i++){
          Sum_x[ch] += pow(new_x[ch][max_i-HALF_WINDOW_SPACING+i], 2);
          
          }
   
   saSNR[ant] = sSNR[ant];
   multi_pulses[ant] = sMulti_pulses[ant];
   }
   
   //saSNR[0] = (sSNR[0] + sSNR[2] +sSNR[4] +sSNR[6]*1.414)/4.;
   //saSNR[1] = (sSNR[1] + sSNR[3] +sSNR[5] +sSNR[7]*1.414)/4.;
...
void get_tdoa(int pol, double gTDOA[6], double tdoa[6]){
   ...
   for(int pair = 0; pair <6; pair++){
	...
	gTDOA[pair] = tdoa[pair] + (peak_position[j] - peak_position[i])*0.8 - (-delay_ant[j] + delay_ant[i])/1000.;
   }
}
...
double real_cross_correlation(int pol, int pair, int i, int j){
        ...
        double Fine_im[16385] = {0};
                       ...
                       for (Int_t k=0; k<HALF_WINDOW_WIDTH; k++){//0 means DC component
...
//double Max = 0.;
	...
	//{
   	       //new_x[i] = 0;	                               
	//}
	Planck_taper_windowing(0.1, 150, &filtered_x[Max_i-HALF_WINDOW_SPACING], &new_x[Max_i-HALF_WINDOW_SPACING]);
return Max_i;
...
int get_FFT(int ch){
   ...
   for(int i=0;i<1500;i++){
	  x[ch][i] = fft_back_x[i]/1500.;
   }

	double data[1500] = {0.};
	//for(int i=0; i<1500; i++){
	//	data[i] = (0.42-0.5*cos(2*PI*(i)/1500.)+0.08*cos(4*PI*(i)/1500.))*x[ch][i];
	//}
	Planck_taper_windowing(0.2, 1500, x[ch], data);
   filter_fft_forward->SetPoints(data);
   filter_fft_forward->Transform();
   filter_fft_forward->GetPointsComplex(Re_ant[ch],Im_ant[ch]);
...
*/
  	   ...
  	   double power[751] = {0.};
  	   for(Int_t k=222; k<421; k++){
  	   		power[k] = Re_ant[ch][k] * Re_ant[ch][k] + Im_ant[ch][k] * Im_ant[ch][k];
  	   		}
...
int find_CW(int ch, bool label[751]){
   ...
   filter_fft_forward->GetPointsComplex(Re_ant[ch],Im_ant[ch]);
	...
	Im_ant[ch][0] = 0;
 	...
 	for (Int_t k=0; k<751; k++){//0 means DC component // 
       if((k<180)||(k>460)) {Re_ant[ch][k] = 0.; Im_ant[ch][k] = 0;}
                             }	
...
for(int i=Max_i-half_pulse_width; i<Max_i+half_pulse_width; i++){
		//if(fabs(filtered_x[ch][i])>pulse_threshold) {count_over_30++;}
		if(((filtered_x[ch][i-1]<30)&&(filtered_x[ch][i]>30))||((filtered_x[ch][i-1]>30)&&(filtered_x[ch][i]<30))||((filtered_x[ch][i-1]>-30)&&(filtered_x[ch][i]<-30))||((filtered_x[ch][i-1]<-30)&&(filtered_x[ch][i]>-30))) {count_over_30++;}
						}
												
//multi_pulses = false;
//if(count_over_30>=5){multi_pulses=true;}
multi_pulses = count_over_30/2;
 ...
 for (Int_t k=0; k<751; k++){//0 means DC component // from 185-350 MHz
       if((k<222)||(k>420)) {Re_ant[ch][k] = 0.; Im_ant[ch][k] = 0;}
  	   ...
  	   for(Int_t k=222; k<421; k++){
  	    ...
  	    //312 276
  	   	sort(&power[240], &power[288]);
  	   	double median = power[264];


	for(Int_t k=222; k<421; k++){
		if(power[k]/median>10){
			//cout<<k<<endl;
			Re_ant[ch][k-1] = 0.;
			Im_ant[ch][k-1] = 0.;
			Re_ant[ch][k] = 0.;
...
double inter_map[120][360] = {0};//theta: -6~90; phi: 0~360
  ...
  //threads[pol*6+pair].join();
    ...
    for(int i_theta=0; i_theta<120; i_theta++){
      for(int i_phi=0; i_phi<360; i_phi++){
          double theta = i_theta - 30.;//cyc
          double phi = i_phi;
   ...
   for(int pair = 0; pair <6; pair++){
	...
	for(int w = 0; w < 3; w++){
    	for(int i_theta=0; i_theta<40; i_theta++){
          ...
          double theta = -2 + i_theta/10. + max_theta[w]-30.;//cyc
    ...
    if(pair == 5) {
     ...
     event_theta[pol] = -2 + fine_max_theta/10. + max_theta[max_w]-30;//cyc
...
double fix_angle_cross_correlation(int pol, double in_theta, double in_phi){	
   ...
   for(int ant = 0; ant < 4; ant++){
   		...
   		TSpline3 *s = new TSpline3("grs", t, remain_x[ch], 1500);
   ...
   double sum_factor[4] = {0.};
   for(int ant=0; ant<4; ant++){
   		for(int k=0; k<15000; k++){
   			sum_factor[ant] += _x[ant][k] * _x[ant][k];
   		}
   		sum_factor[ant] = sqrt(sum_factor[ant]);
   }
   ...
   for(int pair = 0; pair <6; pair++){
    ...
    int ant_j = (j-pol)/2;
    	...
    	double max_sum = 0.;
    	// can not use error, otherwise for V-pol, remain_pulse_cross_correlation will become large
    	for(int error=0; error<1; error++){
    		double sum = 0;
    		int _shift = shift_points + error;
    		for(int k=0; k<15000; k++){
    			int l = k + _shift;
    			if(l<0||l>=15000){continue;}
   				sum += _x[ant_i][k] * _x[ant_j][l];
   			}
   			if(sum>max_sum){max_sum=sum;}
   		}
   		
   		double factor = sum_factor[ant_i] * sum_factor[ant_j];
   		
   		cross_correlator += max_sum/factor;
...
double cross_pol_cross_correlation(int pol, double in_theta, double in_phi){
	...
	for(int ant=0; ant<4; ant++){
		...
		for(int k =0; k<1500; k++){
			...
			remain_x[ch][k] = ((k<peak_position[old_ch]-HALF_WINDOW_SPACING)||(k>peak_position[old_ch]+HALF_WINDOW_SPACING))?0:filtered_x[ch][k];//filtered_x[ch][k];
			}
		}
...
double spectrum_cut(int pol, int run_pos, double V_peak){
	...
	for(int ant=0; ant<4; ant++){
		...
		for(int k=0; k<20; k++){
			...
			double filter_factor = 11.;
			for(int l=-5; l<6; l++){
				int label_k = (10*k+l) + 222;
				filter_factor -= label[label_k];
				}
			filter_factor = 11./filter_factor;
			
			//double power = (Re_ant[ch][_k]*Re_ant[ch][_k] + Im_ant[ch][_k]*Im_ant[ch][_k]);
			double power = (ant_re[ch][_k]*ant_re[ch][_k] + ant_im[ch][_k]*ant_im[ch][_k]);
			power = filter_factor*power;
			...
			if((pps_power == 0)||(power == 0)){
				...
				//50mV -> 6dB
				//
				double threshold = 10*log10(pow(V_peak/60., 2)) + 4;
				if(threshold<4){threshold = 4.;}
				if(threshold>10){threshold = 10.;}
				//cout<<threshold<<endl;
				//threshold = 6.;
				
				if(different>threshold){total_pass++;}
...
void SetTreeBranches(TTree* tr){
	...
	tr->Branch("SNR", rough_SNR, "SNR[4]/D");
	tr->Branch("pulse_power", pulse_power, "pulse_power[4]/D");
	tr->Branch("TDOA", gTDOA, "TDOA[6]/D");
	tr->Branch("theta", reconstructed_theta, "theta[2]/D");
	tr->Branch("phi", reconstructed_phi, "phi[2]/D");
	tr->Branch("trigger_spectrum", trigger_spectrum, "trigger_spectrum[8][20]/D");
	tr->Branch("band_trigger_threshold", band_trigger_threshold, "band_trigger_threshold[8][20]/D");
	tr->Branch("pass_trigger_ratio", pass_trigger_ratio, "pass_trigger_ratio[2]/D");
	tr->Branch("power_frequency", power_frequency, "power_frequency[20]/D");
	tr->Branch("multi_pulses", multi_pulses, "multi_pulses[4]/I");
	tr->Branch("masked_band", &masked_band, "masked_band/I");
	...
	tr->Branch("other_pol_cross_correlation", other_pol_cross_correlation, "other_pol_cross_correlation[2]/D");
		...
		//cout<<"a"<<endl;
		tr->Branch("overVoltBits", &gOverVoltBits);
		//cout<<"a"<<endl;
	#endif
...
void initial_data(){
	...
	for(int pol=0; pol<2; pol++){
		...
		rough_SNR[pol] = -1000;
		reconstructed_theta[pol] = -1000;
		reconstructed_phi[pol] = -1000;
		pass_trigger_ratio[pol] = -1000;
		//multi_pulses[pol] = -1000;
