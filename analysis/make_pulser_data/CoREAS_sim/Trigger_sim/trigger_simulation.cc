...
TFile *ntuple_file = new TFile("./ntuple_16946.root", "update");
...
const Int_t nt_nentries = (Int_t)ntuple->GetEntries() - 1;
  ...
  TBits * gOverVoltBits = NULL;
  		  ...
  		  tr_event->AddFile("/media/cyc/For_Linux/TAROGE4_DATA/data/20210121/run00016946.root"); // /20201218/run00015890
  		  fvNEntries.push_back(tr_event->GetEntries());
  		  //tr_event->AddFile("/media/cyc/For_Linux/TAROGE4_DATA/data/20201218/run00015889.root");
		  //fvNEntries.push_back(tr_event->GetEntries());
	...
	for(int m=0; m<nt_nentries; m++){
      	...
      	if(nt_time<100||nt_time>950){continue;}//198 405  nt_time<1075||nt_time>1150
      	...
      	int t_event_number = (nt_run-16946)*fvNEntries[0] + nt_event;//15888
      ...
      //cout<<j<<endl;
		...
		for(int board=0;board<4;board++){
				...
				for(int band=1; band<9; band++){
					...
					double efficiency = gr_eff_vs_strength[513][board][band-1]->Eval(vpp);
