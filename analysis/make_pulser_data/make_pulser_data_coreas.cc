...
FILE *fpinput = fopen("/home/cyc/programming/taroge/data/simulation_pulse_shape_16ev.txt","r");
float pulse_shape;
...
int pulse_counter=0;
while(1){
 int p = fscanf(fpinput,"%f", &pulse_shape);
if (p!=1) break;
pulse[pulse_counter] = pulse_shape/0.615997;//normalize to 1;
time[pulse_counter] = pulse_counter;
pulse_counter+=1;
...
fclose(fpinput);
 ...
 string out_put_root_name = "run20240202.root";//"make_pulser_data_" + std::to_string(pol) + ".root";
   ...
   //also store the pps original data
	...
	for(int ant=0; ant<4; ant++){
		...
		for(int k=400; k<500; k++){
			new_x[ch][k] += ant_4_factor*50*pulse[k-400];
			new_x[o_ch][k] += ant_4_factor*0.4*50*pulse[k-400];
		}
	}
