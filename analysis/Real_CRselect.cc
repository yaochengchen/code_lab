...
//int event_record[200000][2] = {0};// run, entry
...
//double *event_time = new double[200000];
double event_theta[2][200000] = {0};
double event_phi[2][200000] = {0};
...
nt_j=0;
  ...
  //continue;
    ...
    fname_evt =  Form("/media/cyc/For_Linux/TAROGE4_DATA/data/20210428/run%08d.root",number);//14
...
for (int entry = 0; entry<event_number/20; entry++){



      Tree_Muon->GetEntry(entry*20);

if((number==number1)&&(entry==0)) eventTime_start = eventTime;
...
//event_record[nt_j][0] = number;
//event_record[nt_j][1] = entry;

if(inter_max[1]>4.2){

//cout<<event_phi[1][nt_j]<<"   "<<event_theta[1][nt_j]<<endl;

if((event_phi[1][nt_j]<-69)&&(event_phi[1][nt_j]>-71)&&(event_theta[1][nt_j]>3)&&(event_theta[1][nt_j]<7)&&a_count<10){cout<<1<<"    "<<number<<"    "<<entry<<endl; a_count++;}

if((event_phi[1][nt_j]<-55)&&(event_phi[1][nt_j]>-65)&&(event_theta[1][nt_j]>29)&&(event_theta[1][nt_j]<32)&&b_count<10){cout<<2<<"    "<<number<<"    "<<entry<<endl; b_count++;}

if((event_phi[1][nt_j]<-45)&&(event_phi[1][nt_j]>-50)&&(event_theta[1][nt_j]>39)&&(event_theta[1][nt_j]<42)&&c_count<10){cout<<3<<"    "<<number<<"    "<<entry<<endl; c_count++;}

if((event_phi[1][nt_j]<-55)&&(event_phi[1][nt_j]>-60)&&(event_theta[1][nt_j]>8)&&(event_theta[1][nt_j]<12)&&d_count<10){cout<<4<<"    "<<number<<"    "<<entry<<endl; d_count++;}

if((event_phi[1][nt_j]<-45)&&(event_phi[1][nt_j]>-50)&&(event_theta[1][nt_j]>0)&&(event_theta[1][nt_j]<4)&&e_count<10){cout<<5<<"    "<<number<<"    "<<entry<<endl; e_count++;}

if((event_phi[1][nt_j]<-27)&&(event_phi[1][nt_j]>-33)&&(event_theta[1][nt_j]>4)&&(event_theta[1][nt_j]<9)&&f_count<10){cout<<6<<"    "<<number<<"    "<<entry<<endl; f_count++;}

if((event_phi[1][nt_j]<-35)&&(event_phi[1][nt_j]>-40)&&(event_theta[1][nt_j]>18)&&(event_theta[1][nt_j]<24)&&g_count<10){cout<<7<<"    "<<number<<"    "<<entry<<endl; g_count++;}

   if(nt_j%1000==0) cout<<nt_j<<endl;
...
*/
     ...
     cyc[pol]= new TGraph(nt_j, event_phi[pol], event_theta[pol]);
   
     cyc[pol]->Draw("AP");
     cyc[pol]->GetYaxis()->SetTitle("Theta");
