...
double pps_average[8][20] = {0.};
double pps_median[8][20] = {0.};
		...
		if(entry == max){
			for(int ch=0; ch<8; ch++){
				for(int k=0; k<20; k++){//199
					...
					pps_median[ch][k] = median[ch][k][num_pps/2];
					pps_average[ch][k] = TMath::Mean(num_pps, median[ch][k]);
...
filtering(saSNR, mul_pulses);
	...
	int pol = 0;
	if(V_trigger_bit>H_trigger_bit){pol = 1;}
	//for(int pol=0; pol<2; pol++){
		...
		if(true){
    		...
    		pass_trigger_ratio[pol] = spectrum_cut(pol, run_pos, rough_SNR[pol]);
    		...
    		if(true){
				...
				reconstructed_theta[pol] = event_theta[nt_j][pol];
				reconstructed_phi[pol] = event_phi[nt_j][pol];
...
//barson
	//if(rough_SNR[1]>100&&nt_j%100==0){
	if(false){
	//out_put<<nt_j<<"   "<<rough_SNR[1]<<endl;
	cout<<which_date<<"    "<<event_record[nt_j][0]<<"    "<<event_record[nt_j][1]<<"    "<<x_value[nt_j][0]<<"    	"<<event_theta[0][nt_j]<<"    "<<event_phi[0][nt_j]<<"    "<<x_value[nt_j][1]<<"    "<<event_theta[1][nt_j]<<"    	"<<event_phi[1][nt_j]<<endl;
	out_put<<which_date<<"    "<<event_record[nt_j][0]<<"    "<<event_record[nt_j][1]<<"    "<<x_value[nt_j][0]<<"    	"<<event_theta[0][nt_j]<<"    "<<event_phi[0][nt_j]<<"    "<<x_value[nt_j][1]<<"    "<<event_theta[1][nt_j]<<"    	"<<event_phi[1][nt_j]<<endl;
	}
	...
	for(int ant=0; ant<4; ant++){
		...
		for(int k=0; k<20; k++){
			...
			if((pps_power == 0)||(power == 0)){
				...
				//50mV -> 6dB
				//
				double threshold = 10*log10(pow(V_peak/60., 2)) + 6;
...
void SetTreeBranches(TTree* tr){
	...
	tr->Branch("pps_median",&pps_median, "pps_median[8][20]/D");
	tr->Branch("pps_average",&pps_average, "pps_average[8][20]/D");
