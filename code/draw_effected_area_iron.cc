	...
	for(int k=0; k<8; k++){
		for(int i_phi = 0; i_phi<19; i_phi++){
			...
			for(double theta = 89.99; theta>=40.01; theta-=0.02){
				Ave_theta[k][i_phi] += (TMath::Cos((theta-0.01)*TMath::DegToRad())-TMath::Cos((theta+0.01)*TMath::DegToRad()))*wyn->Eval(theta)/(TMath::Cos((40)*TMath::DegToRad())-TMath::Cos((90)*TMath::DegToRad()));
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	wyn->GetYaxis()->SetTitle("Acceptance [km^{2} sr]");
	...
	for(int k=0; k<6; k++){
		for(int i=0; i<8; i++){
			sum_1[i] += Bin_theta_Bin_E[1][k][i];
		}
	}
	for(int i=0; i<8; i++){cout<<sum_1[i]<<endl;}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	for(int k=0; k<6; k++){
		for(int i=0; i<8; i++){
			sum_2[i] += Bin_phi_Bin_E[1][k][i];
		}
	}
	for(int i=0; i<8; i++){cout<<sum_2[i]<<endl;}
