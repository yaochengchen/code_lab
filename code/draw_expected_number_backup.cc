...
void draw_expected_number(){
	...
	TFile * f_ntuple_analysis = new TFile("./ntuple_analysis_efficiency.root");
	...
	for(int i =0; i<ntuple[7]->GetEntries(); i++){
			...
			int	i_theta = (i/19)%9;
			ntuple[7]->GetEntry(i);
			double final_EffectedArea = _EffectedArea*1.0e-6*analysis_efficiency[7][i_theta][i_phi];
			double final_Triggered_count = _Triggered_count*analysis_efficiency[7][i_theta][i_phi];
			fitting_APT[i_phi][i_theta] = final_EffectedArea;
			double _Error_EffectedArea = 0.;
			if(final_Triggered_count>0){_Error_EffectedArea = _EffectedArea/sqrt(final_Triggered_count);}
	...
	for(int n=0; n<16; n++){
		...
		for(int i =0; i<ntuple[n]->GetEntries(); i++){
			...
			EffectedArea[i_theta][i_phi][k] = final_EffectedArea;
			EffectedArea_tp[k][i_theta][i_phi] = final_EffectedArea;
			EffectedArea_pt[k][i_phi][i_theta] = final_EffectedArea;
